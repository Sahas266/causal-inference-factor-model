...
def render_table(title: str, rows: list[SweepRow], baseline: dict) -> str:
    lines = [f"### {title}\n"]
    lines.append(f"Baseline (BH_BTC over OOS): total {baseline['total_return']:+.1%}, "
    ...
    for row in rows:
        ...
        if m.get("n_obs", 0) == 0:
            ...
            continue
        beats = "✓" if m["total_return"] > baseline["total_return"] else "—"
        lines.append(
            f"| {row.label} | {m['n_obs']} | {m['total_return']:+.1%} | "
            f"{m['sharpe']:.3f} | {m['sortino']:.3f} | {m['max_dd']:+.1%} | "
            f"{m['avg_turnover']:.3f} | {beats} |"
        )
    lines.append("")
    return "\n".join(lines)
...
def main():
    ...
    returns, macro = load_returns_and_macro(assets, args.start, args.end)
    ...
    bh_oos = slice_metrics(bh, returns.index, oos_s, oos_e)
    ...
    chunks = [
        ...
        f"BH_BTC baseline: total {bh_oos['total_return']:+.1%}, Sharpe {bh_oos['sharpe']:.3f}, MaxDD {bh_oos['max_dd']:+.1%}.  "
