"""Thread-safe rate limiter for Allium API"""
...
class AlliumRateLimiter(RateLimiterInterface):
    """
    Token bucket rate limiter for Allium API.

    Allium default limits: 60 requests per minute (1 req/sec).
    Pro/Enterprise tiers may have higher limits.
    """
    ...
    def __init__(
        ...
        logger.info(
            f"Allium rate limiter: {self.requests_per_window} req/{window_seconds}s "
    ...
    def reset(self) -> None:
        with self.lock:
            ...
            self.total_requests = 0
            self.total_wait_time = 0.0
            logger.debug("Allium rate limiter reset")
