...
class CoinGeckoRateLimiter(RateLimiterInterface):
    """
    Token bucket rate limiter for CoinGecko API.
    ...
    def __init__(
        ...
        logger.info(
            f"CoinGecko rate limiter: {self.requests_per_window} req/{window_seconds}s "
    ...
    def reset(self) -> None:
        with self.lock:
            ...
            self.total_requests = 0
            self.total_wait_time = 0.0
            logger.debug("CoinGecko rate limiter reset")
