"""Thread-safe rate limiter for DefiLlama API"""
...
class DefiLlamaRateLimiter(RateLimiterInterface):
    """
    Token bucket rate limiter for DefiLlama API.

    Free tier: ~300 requests per minute.
    Pro tier: higher limits.
    Default: 200 requests per minute with 0.9 safety margin.
    """
    ...
    def __init__(
        ...
        logger.info(
            f"DefiLlama rate limiter: {self.requests_per_window} req/{window_seconds}s "
